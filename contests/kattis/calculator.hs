...
main :: IO ()
main =
  interact $ lines >>> map (solve >>> showFloat) >>> unlines

solve :: String -> Double
solve = evalExpr . parseExpr

parseExpr :: String -> Expr
parseExpr = convertEtoExpr . either undefined id . parse parseE "" . filter (/= ' ')
  where
    parseE = EExpr <$> parseF <*> parseR
    parseR = (RAdd <$> (char '+' *> parseF) <*> parseR) <|> (RSub <$> (char '-' *> parseF) <*> parseR) <|> return REps
    parseF = FExpr <$> parseT <*> parseS
    parseS = (SMul <$> (char '*' *> parseT) <*> parseS) <|> (SDiv <$> (char '/' *> parseT) <*> parseS) <|> return SEps
    parseT = option id (char '-' $> TNeg) <*> ((TWrap <$> (char '(' *> parseE <* char ')')) <|> (TNum . read <$> many digit))
...
convertEtoExpr (EExpr f r) = foldl (flip (.)) id (convertRtoExprs r) (convertFtoExpr f)

-- R = `+` F R | `-` F R | eps
data R = RAdd F R | RSub F R | REps
...
convertFtoExpr (FExpr t s) = foldl (flip (.)) id (convertStoExprs s) (convertTtoExpr t)

-- S = `*` T S | `/` T S | eps
data S = SMul T S | SDiv T S | SEps
...
-- T = `-`? { `(` E `)` | digit* }
data T = TNeg T | TWrap E | TNum Int

...
solve = reverse . catMaybes . snd . foldl eval (emptyContext, []) . map parse1
  where
    eval (gamma, ys) st = let (gamma', s) = evalStatement gamma st in (gamma', s : ys)
...
evalExpr gamma (Var s) = let (Just v) = M.lookup s gamma in v
evalExpr _ (Lit s) = s
